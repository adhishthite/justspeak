...
- `21-hud` — notch-anchored pill on a STATIC max-footprint panel; ALL motion is `SpringChannel` (semi-implicit Euler, substepped for stability — Python-verify any retuning) driving view frames from a display-link tick (CADisplayLink macOS 14+, 60Hz timer fallback; glow/orb advance capped ~40Hz). Never animate the panel frame — window-server frame animation was the old smoothness ceiling. Reveal styles are pure presence→geometry mappings (top edge pinned) sharing one retargetable spring; truncation rides in the attributed string's NSParagraphStyle (cell `lineBreakMode` is ignored for attributed values).
