...
- `08-audio-engine` — always-on AVAudioEngine (input pinned to `INPUT_DEVICE` via `kAudioOutputUnitProperty_CurrentDevice` on the inputNode's AUHAL before prepare; `currentInput` is main-thread-only and re-resolved on every configuration change — `29-input-devices` does the CoreAudio enumeration/matching) with pre-roll ring buffer; per-buffer RMS while recording feeds the adaptive trailing capture in `stopRecording` (quiet-window `POST_ROLL_MS`, cap `POST_ROLL_MAX_MS`); `duration` is measured at entry (micro-click guard depends on it); input-device changes (`AVAudioEngineConfigurationChange`, main thread) rebuild tap + converter and restart only if the engine was meant to be running.
