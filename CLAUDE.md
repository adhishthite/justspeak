...
- `07-history-store` — SQLite at `~/.justspeak/history.db` (WAL, 0600); one row per turn (success/empty/error) incl. tokens, cost, latencies, frontmost app; all writes are fire-and-forget `queue.async` **after** the paste; schema changes need a best-effort `ALTER TABLE` migration (duplicate-column errors deliberately ignored); INSERT columns/placeholders/binds must stay aligned (currently 38).
- `08-audio-engine` — always-on AVAudioEngine (input pinned to `INPUT_DEVICE` via `kAudioOutputUnitProperty_CurrentDevice` on the inputNode's AUHAL before prepare; `currentInput` is main-thread-only and re-resolved on every configuration change; `INPUT_DEVICE=auto` re-pins on lid flips via `didChangeScreenParametersNotification` → `reselectForLidState`, deferred past an in-flight turn through `applyPendingReselect` in `scheduleMicIdleRelease`; a re-pin requires the engine stopped first — `29-input-devices` does the CoreAudio enumeration/matching and the IOKit `AppleClamshellState` read) with pre-roll ring buffer; per-buffer RMS while recording feeds the adaptive trailing capture in `stopRecording` (quiet-window `POST_ROLL_MS`, cap `POST_ROLL_MAX_MS`; quiet already banked before key-up is read off the tail of `frameDbValues` so a finished speaker pays only the 60ms `minTrailSec` floor, not the whole window); `duration` is measured at entry (micro-click guard depends on it); input-device changes (`AVAudioEngineConfigurationChange`, main thread) rebuild tap + converter and restart only if the engine was meant to be running.
...
- `17-hud-aura` — `strokeHalo` (crisp rim + two offset-shadow gaussian passes, Talkify's glow construction) serves both geometries: the dilated notch path and, in `pillMode`, the even-odd-clipped pill capsule; the pill panel margin in `21-hud` must exceed the halo's worst-case reach (~75px with the pill-mode flank/underside spread pass) or the fade gets a panel-edge cutoff. Design reference for notch HUD treatments: https://github.com/tornikegomareli/Talkify (SwiftUI — `HUDEdgeGlowView` has the glow recipe: crisp stroke at w/2 + two blurred copies at blur r and r/2, voice level swelling width and blur; also open-silhouette stroking that never draws the hidden top edge, and sweep/particle ideas we haven't ported).
- `21-hud` — notch-anchored pill on a STATIC max-footprint panel; ALL motion is `SpringChannel` (semi-implicit Euler, substepped for stability — Python-verify any retuning) driving view frames from a display-link tick (CADisplayLink macOS 14+, 60Hz timer fallback; glow/orb advance capped ~40Hz). Never animate the panel frame — window-server frame animation was the old smoothness ceiling. Reveal styles are pure presence→geometry mappings sharing one retargetable spring (top edge pinned, except morph's phase-B detach — the host panel spans to the screen top so the membrane can start inside the notch cutout); truncation rides in the attributed string's NSParagraphStyle (cell `lineBreakMode` is ignored for attributed values).
- `21-hud` (cont.) — hold-to-lock rim: `HoldRingView` (a subview whose backing layer IS the `CAShapeLayer` via `makeBackingLayer` — never a hand-added sublayer, AppKit reorders those under the backplate) whose `strokeEnd` rides the wall clock from `showListening(lockAfter:)`'s `listenStart` (not a spring — it must close when the app's lock timer fires); `showLocked()` flares it via the `lockPulse` spring, swaps the orb to the padlock, and holds the hint copy 1.8s while live words queue in `deferredLiveText` (flushed from `tick`); every non-listening state calls `clearLockRing()`.
- `21-hud` (cont.) — `retarget(to:)` re-anchors on the display `30-focus-screen` resolves at key-down (AX focused-window bounds of the frontmost pid, 50ms messaging timeout, pointer fallback); the lookup runs in the async main hop *after* `startRecording`, never before it; `NSView.displayLink` follows the panel's screen on its own.
...
- `28-correction-watcher` — opt-in (`LEARN_CORRECTIONS`) post-paste typed-correction capture: armed on main after a successful (non-copy-only) paste, one AX read of the focused field after `LEARN_DELAY_MS`, token-window locate + LCS diff + hard gates (single-word swaps, ≥3 chars, no caps-only, proper-noun-or-vocab right side, Levenshtein band, max 3) — Python-verify any gate/diff retuning; `generation` is main-thread-only (bumped by `arm`/`cancelPending`; every capture start cancels the pending read); observations go to the `corrections` table (word pairs only, never field content) for the analyzer — nothing is auto-applied.
- `25-analyzer` — `--analyze` / `make analyze`: mines the history DB (success rows with `ts_epoch`+`app_name`, newest-first, 300k-char payload cap) for vocab suggestions via one REST call (`responseMimeType: application/json`, `ANALYZE_MODEL`, default `gemini-3.7-flash`); a deterministic pre-pass flags re-dictation pairs (Δt ≤ 120s, token-Jaccard ≥ 0.5) as correction evidence; the existing vocabulary is deliberately the *target list* for `wrong => right` hunting (right side need not appear in transcripts), with `ANALYZE_CONTEXT` / vocabulary-file `# context:` persona sharpening inference and the `corrections` table (from `28-correction-watcher`, read leniently — absent table = silence) supplying ground truth; replacement suggestions are gated ≤4 words/side, never caps-only; suggestions are rendered in vocabulary-file syntax so the interactive append is a plain string concat `parseVocabulary` already reads; strictly user-run — never wired into the dictation path.

## Invariants that past bugs were made of

- **Hold-to-lock** (`turnLocked`, `lockWorkItem`, `lockLimitWorkItem` — main-thread-only): the lock item is armed after `captureActive = true` and re-checks `captureActive` when it fires; a locked `handleKeyUp` returns *before* clearing `captureActive` (so the finishing press can reuse the normal key-up path), and `handleKeyDown` checks `turnLocked` *first* — before the `isProcessing` guard — then calls `handleKeyUp()` itself; the physical release after that press is dropped by the `captureActive` guard. `handleKeyUp(finish:)` stamps `turnFinishMode` (`release` / `lock_press` / `lock_limit` → history `finish_mode`). Off in toggle mode (`holdToLockInterval`).
