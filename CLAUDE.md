...
- **Hold-to-lock** (`turnLocked`, `lockWorkItem`, `lockLimitWorkItem` — main-thread-only): the lock item is armed after `captureActive = true` and re-checks `captureActive` when it fires; a locked `handleKeyUp` returns *before* clearing `captureActive` (so the finishing press can reuse the normal key-up path), and `handleKeyDown` checks `turnLocked` *first* — before the `isProcessing` guard — then calls `handleKeyUp()` itself; the physical release after that press is dropped by the `captureActive` guard. Off in toggle mode (`holdToLockInterval`).
